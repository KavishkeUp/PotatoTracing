...
// Function to create harvest record directly in database
async function createHarvestDirect(farmer, harvestData) {
    try {
        ...
        const savedHarvest = await harvestDoc.save();
        console.log('✅ Created harvest:', savedHarvest.harvestId);
        return savedHarvest.harvestId;
    } catch (error) {
...
// Function to create collection record directly in database
async function createCollectionDirect(harvestId, clerk) {
    try {
        ...
        const savedCollection = await collectionDoc.save();
        console.log('✅ Created collection:', savedCollection.collectionId);
...
// Function to create distribution record directly in database
async function createDistributionDirect(collectionId, distributor) {
    try {
        ...
        const savedDistribution = await distributionDoc.save();
        console.log('✅ Created distribution:', savedDistribution.distributionId);
        return savedDistribution.distributionId;
    } catch (error) {
...
// Function to create supermarket record directly in database
async function createSupermarketDirect(distributionId, supermarket) {
    try {
        const supermarketDoc = new Supermarket({
            ...
            qrCode: `https://example.com/qr/${distributionId}`,
            owner: 'seeding-script'
        });

        const savedSupermarket = await supermarketDoc.save();
        console.log('✅ Created supermarket record:', savedSupermarket.supermarketId);
        return savedSupermarket.supermarketId;
    } catch (error) {
        console.error('❌ Error creating supermarket record:', error.message);
        return null;
    }
}

// Function to create complete traceability chain
async function createTraceabilityChainDirect(excelData) {
    const results = [];
    
    for (let i = 0; i < Math.min(excelData.length, 100); i++) { // Create 100 complete chains
        ...
        console.log(`\n🌱 Creating Traceability Chain ${i + 1}/100`);

        // Create harvest
        const harvestId = await createHarvestDirect(farmer, data);
        if (!harvestId) continue;

        // Create collection
        const collectionId = await createCollectionDirect(harvestId, clerk);
        if (!collectionId) continue;

        // Create distribution
        const distributionId = await createDistributionDirect(collectionId, distributor);
        if (!distributionId) continue;

        // Create supermarket record
        const supermarketId = await createSupermarketDirect(distributionId, supermarket);
        ...
        console.log(`   ${farmer.name} → ${clerk.name} → ${distributor.name} → ${supermarket.name}`);
...
async function seedDataDirect() {
    try {
        ...
        if (excelData.length > 0) {
            console.log('📋 Sample data structure:', Object.keys(excelData[0]));
        }

        // Clear existing data (optional - comment out if you want to keep existing data)
        console.log('🧹 Clearing existing data...');
        await Harvest.deleteMany({ owner: 'seeding-script' });
        await Collection.deleteMany({ owner: 'seeding-script' });
        await Distribution.deleteMany({ owner: 'seeding-script' });
        await Supermarket.deleteMany({ owner: 'seeding-script' });
        console.log('✅ Existing data cleared');

        console.log('\n🌱 Creating traceability chains...');
        const results = await createTraceabilityChainDirect(excelData);
...
if (require.main === module) {
    seedDataDirect()
        .then(() => {
            console.log('\n✨ Direct seeding script completed successfully!');
            process.exit(0);
        })
        .catch((error) => {
            console.error('\n💥 Direct seeding script failed:', error);
            process.exit(1);
        });
}
